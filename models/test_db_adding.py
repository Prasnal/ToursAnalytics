...
def add_data_to_database(obj, scraped_date):
    with Session() as session:
        ...
        tour_obj = add_tour(obj, session, tour_type_o, agency_obj)
...
def add_tour_price(detail, session, tour_config_obj, scraped_date):
    ...
    tour_price_o = get_or_create(session, TourPrice, scraped_date=scraped_date,
...
def add_tour_config(detail, session, tour_obj):
    ...
    tour_config_o = get_or_create(session, TourConfig, defaults={"tour_prices": []}, tour_length=tour_length,
                                start_location=start_location,
                                location_additional_cost=location_additional_cost,
                                start_tour_date=start_tour_date, end_tour_date=end_tour_date, tour=tour_obj[0],
...
def add_tour(obj, session, tour_type_obj, tour_agency_obj):
    tour_id = obj.tour_id
    omnibus_key = obj.klucz_omnibus
    tour_name = obj.tour_name
    tour_url = obj.tour_url
    tour_obj = get_or_create(session, Tour, defaults={'tour_photos': [], 'tour_config': [], 'omnibus_key': omnibus_key}, original_tour_id=tour_id,
                              tour_name=tour_name, tour_url=tour_url,
                             tour_type=tour_type_obj[0], tour_type_id=tour_type_obj[0].id,
                             tour_agency=tour_agency_obj[0], tour_agency_id=tour_agency_obj[0].id)
    return tour_obj
...
def add_photos(obj, session, tour_obj):
    ...
    for photo_url in obj.photos:
        try:
            ...
            print("ERROR ADDING PHOTO TO DATABASE:", photo_url)
            continue
    return photo_objs
...
def add_countries(obj, session):
    ...
    for country in countries:
        countries_objs.append(get_or_create(session, Country, country_name=country, country_code=country))
